...
if ($_SESSION['error_login'] == "") 
	...
	{
		$Email=$_REQUEST['Email']; //Recogemos usuario y contraseña
		$Password=$_REQUEST['Password'];
			...
			if (($Email=="") || ($Password=="")) //Error campos en blanco
          	{
				...
				$url_relativa = "ingresoCliente.php"; //La dirección de login para el header
			...
			else//Si no es en blanco comprobamos de nuestra base de datos
			{
				...
				$row = mysql_fetch_array($sql);
					if($row>0)
					{ 
					if($row['1'] == $Password && $row['2']==1 && $row['3']==1)
					{
						...
						$_SESSION['status']=1;
						$url_relativa = "clientePage.php"; //Si todo es valido dejo entrar
						header("Location: http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/" .$url_relativa);
					...
					else//Si falla la contraseña, error			
					{
						...
						$url_relativa="ingresoCliente.php";
						header("Location: http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/" .$url_relativa);
					}//FIN ELSE
			}//FIN ELSE PRINCIPAL
			else//Si falla el usuario, error
			{
				...
				$url_relativa="validar_cliente.php";
				header("Location: http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/" . $url_relativa);
			}//fin else
